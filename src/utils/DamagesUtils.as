package utils 
{
	...
	{
		private static var _sysApi:SystemApi;
		private static var _fightApi:FightApi;
		...
		/**
		 * Compute the damages of a Spell or a Weapon.
		 * 
		...
		public static function computeDamages(spellOrWeapon:Object, targetInfos:GameFightFighterInformations, distance:int = 0):SpellDamages
		{
			...
			if (spellOrWeapon is SpellWrapper)
			{
				return computeDamagesSpell(spellOrWeapon as SpellWrapper, targetInfos, distance % 5);
			}
			else if (spellOrWeapon is WeaponWrapper)
			{
				return computeDamagesWeapon(spellOrWeapon as WeaponWrapper, targetInfos, distance % 2);
			}
		...
		{
			var characterStats:CharacterCharacteristicsInformations = _fightApi.getCurrentPlayedCharacteristicsInformations();
			
			var targeterTeam:String = _fightApi.getFighterInformations(targetInfos.contextualId).team;
			var targetTeam:String = _fightApi.getFighterInformations(_fightApi.getCurrentPlayedFighterId()).team;
			
			var isTargetMe:Boolean = (targetInfos.contextualId == _fightApi.getCurrentPlayedFighterId());
			...
			for each(var mask:String in masks)
			{
				switch(mask)
				{
					...
					case TargetMaskEnum.ALL_ENEMY:
						...
						break;
					default:
						_sysApi.log(2, "Unknow mask : " + mask);
		...
		private static function computeInitialDamage(effect:EffectInstance, characterStats:CharacterCharacteristicsInformations, isCriticalDamage:Boolean = false, criticalBonus:int = 0, skillBonus:int = 0):EffectDamages
		{
			...
			switch (effect.effectId)
			{
				...
				case EffectIdEnum.ATTACK_PUSHBACK:
					var characterLvl:int = _fightApi.getFighterLevel(_fightApi.getCurrentPlayedFighterId());
		...
		public static function getSkillBonus(fighterId:int = 0):int
		{
			if (fighterId == 0)
			{
				fighterId = _fightApi.getCurrentPlayedFighterId();
			}
			
			for each(var buff:Object in _fightApi.getAllBuffEffects(fighterId).buffArray[BuffEffectCategoryEnum.ACTIVE_BONUS])
			{
		...
		{
			var level:int = _fightApi.getFighterLevel(target.contextualId);
